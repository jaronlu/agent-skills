...
- A second level is normally a category, not a domain. Open a domain folder only when that one domain keeps four or more bookmarks; leaves inside it drop the `area` prefix.
...
- **dev**: one subfolder per platform or stack the user actually uses, added only when that platform keeps four or more bookmarks; leaves inside it drop the `area` prefix. Official docs and repository roots belong here.
- **ai/agent**: model labs, agent frameworks, coding agents, MCP, and the repositories behind them, one leaf per `area·slug`.
- **ai/relay**: third-party API relays, coding-plan aggregators, shared-account services, kept separate from `ai/agent`. Prefer one leaf per relay vendor (`area·slug`).
...
Name every saved URL `<area>·<slug>` on every Organize: an area token, a middle dot, then a short slug for the product or page (lowercase, hyphens instead of spaces). A user folder scheme does not replace this unless the user also names a different leaf pattern. Pick exactly one area — company, platform, or topic — from why the page was saved. When candidate areas nest, prefer the subset: `Apple ⊃ ios ⊃ uikit` → `ios·uikit`, never `Apple·ios·uikit` or `Apple·uikit`.

- **company** — vendor, site, or organization; keep brand casing. `OpenAI·api`, `Anthropic·claude-code`, `Vercel·docs`
- **platform** — language, OS, or runtime; lowercase. `python·asyncio`, `ios·uikit`, `android·compose`
- **topic** — subject that is neither company nor platform; lowercase. `algo·dp`, `ml·attention`, `tax·vat`

The pattern targets product, site, repository, and reference leaves. Personal documents, government pages, internal systems, and anything the user named keep their own title — a labour-claim document stays recognisable as `劳动维权·赔偿估算` rather than being flattened into `Google·document`.

- The same `area·slug` appears once per folder: replace the older URL instead of keeping a second one, and prefer the official documentation URL or the repository root over pricing, tag, dashboard, and session URLs.
